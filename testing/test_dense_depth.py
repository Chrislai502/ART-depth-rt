...
def show_depth_images(depth_images):
    """
    Display a batch of images vertically stacked using matplotlib.
    ...
    """
    # batch_images = torch.stack([unnormalize(img) for img in batch_images])  # Unnormalize all images
    depth_images = torch.stack([img for img in depth_images])  # Unnormalize all images
    
    depth_images = torch.clamp(depth_images, 0, 255)  # Clip values to [0, 1]

    # Vertically stack all images in the batch
    vstacked_image = vstack_batch_images(depth_images)
...
def test_data_loading(config, dataset_class, sample_ratio=None, mode="train"):
    ...
    if sample_ratio:
        ...
        data_loader = dataset_class(config.dataset, mode, sample_ratio, device=device).data
    else:
        data_loader = DepthDataLoader(config.dataset, config.dataset.art.dataset, mode, device=device).data
...
def main(cfg: DictConfig):
    ...
    test_data_loading(config=cfg, dataset_class=DepthDataLoader, mode=mode) # Single Dataset
